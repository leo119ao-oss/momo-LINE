...
import { lineClient } from '@/lib/lineClient';      // lineClient はこちらから呼び出す
...
async function handleImage(event: MessageEvent){
  ...
  try {
    ...
    const caps = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        { role: 'system', content: '次の文から、日記のキャプション候補を日本語で3つ。15〜28字。言い切り or 〜だなあ調。絵文字や記号 なし。' },
      ...
      if (event.type === 'message' && event.message.type === 'text') {
        ...
        try {
          ...
          if(pending && pending.length){
            const m = pending[0];
            const openai = new (await import('openai')).default({ apiKey: process.env.OPENAI_API_KEY! });
            const sys = 'あなたは写真日記の編集者。ユーザーの一言 を活かし、やさしい1文キャプションを作る。';
            const prompt = `画像 メモ: ${m.guess}\nユーザーの追記: ${userMessage}\n→ 30〜60字の一文キャプションで。`;
          ...
          try {
            await lineClient.replyMessage(event.replyToken, {
              type: 'text',
              text: 'すみません、一時的にエラーが発生しました。しばらく時間をおいてから、もう一度 お試しください。',
            });
          } catch (replyError) {
