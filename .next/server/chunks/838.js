...
— 参考記事 —
${w}`}catch(e){return console.error("RAG process failed:",e),"申し訳ありません、情報の検索中にエラーが発生しました。もう一度お試しください。"}}async function k(e,t){let r;let s=await w(e);await n.supabaseAdmin.from("chat_logs").insert({participant_id:s.id,role:"user",content:t});let{data:o}=await n.supabaseAdmin.from("media_entries").select("*").eq("participant_id",s.id).eq("status","awaiting").order("created_at",{ascending:!1}).limit(1);if(o?.length){let e=o[0],r=new(await Promise.resolve().then(a.bind(a,4214))).default({apiKey:process.env.OPENAI_API_KEY}),i=`画像メモ: ${e.guess}
ユーザーの追記: ${t}
→ 30〜60字の一文キャプションで。`,s=await r.chat.completions.create({model:"gpt-4o-mini",messages:[{role:"system",content:"あなたは写真日記の編集者。ユーザーの一言を活かし、やさしい1文キャプションを作る。"},{role:"user",content:i}],temperature:.3}),l=s.choices[0].message.content?.trim()||t.trim();return await n.supabaseAdmin.from("media_entries").update({user_answer:t,caption:l,status:"done"}).eq("id",e.id),`できたよ。
「${l}」
（このあと一覧ページも作れるようにするね）
${e.image_url}`}let{data:l}=await n.supabaseAdmin.from("pending_intents").select("*").eq("participant_id",s.id).eq("kind","web_search").gt("expires_at",new Date().toISOString()).order("created_at",{ascending:!1}).limit(1),c=/^(はい|うん|ok|お願いします|お願い|調べて|いいよ)/i.test(t.trim()),m=/^(いいえ|いらない|大丈夫|結構)/i.test(t.trim());if(l&&l.length){if(c){await n.supabaseAdmin.from("pending_intents").delete().eq("id",l[0].id);let e=new URL("http://localhost:3000");e.pathname="/api/search/google",e.searchParams.set("q",l[0].payload?.query||t);let r=await fetch(e.toString()),{items:i}=await r.json();if(!i?.length)return"検索してみたけれど、めぼしい情報は見つからなかったよ。";let s=i.slice(0,3).map((e,t)=>`[${t+1}] ${e.title}
${e.snippet}
${e.link}`).join("\n\n"),o=new(await Promise.resolve().then(a.bind(a,4214))).default({apiKey:process.env.OPENAI_API_KEY});return(await o.chat.completions.create({model:"gpt-4o-mini",temperature:.2,messages:[{role:"system",content:"あなたはリサーチアシスタント。上の候補を3行で要約し、最後に「次の一歩」を1文添える。装飾不可。"},{role:"user",content:s}]})).choices[0].message.content||s}if(m)return await n.supabaseAdmin.from("pending_intents").delete().eq("id",l[0].id),"了解。また必要になったら声かけてね。"}let u=await A(t),p=/[？\?]/.test(t)?"information_seeking":"personal_reflection"===y&&t.length<25?"personal_reflection":u;if(y=p,console.log(`[Intent] User message: "${t}" -> Raw: ${u} -> Final: ${p}`),"information_seeking"===p)r=await b(s,t);else{console.log("Handling personal reflection intent...");let{data:e}=await n.supabaseAdmin.from("chat_logs").select("role, content").eq("participant_id",s.id).order("created_at",{ascending:!1}).limit(9),a=(e||[]).reverse().map(e=>({role:"ai"===e.role?"assistant":"user",content:e.content}));a.push({role:"user",content:t});let o=s.profile_summary?`
[ユーザープロフィール要約]
${s.profile_summary}
`:"",l=`
${i}${o}
