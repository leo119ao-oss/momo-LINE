"use strict";exports.id=838,exports.ids=[838],exports.modules={838:(e,t,a)=>{a.d(t,{U0:()=>g,zb:()=>R});var n=a(8316),r=a(4214);let o=`
...
4) 箇条書きOK。最後に一言だけ励ます。
5) コンテキスト外は無理に答えない。
`.trim(),h=(await f.chat.completions.create({model:"gpt-4o-mini",temperature:.5,messages:[{role:"system",content:d},{role:"user",content:`コンテキスト:
${u}

質問: ${t}

直前ユーザー発話: ${m}`}]})).choices[0].message.content||"すみません、うまくお答えできませんでした。",w=await g(t,c);return`${h}

— 参考記事 —
${w}`}catch(e){return console.error("RAG process failed:",e),"申し訳ありません、情報の検索中にエラーが発生しました。もう一度お試しください。"}}async function R(e,t){let a;let r=await w(e);await n.supabaseAdmin.from("chat_logs").insert({participant_id:r.id,role:"user",content:t});let i=await A(t),s=/[？\?]/.test(t)?"information_seeking":"personal_reflection"===y&&t.length<25?"personal_reflection":i;if(y=s,console.log(`[Intent] User message: "${t}" -> Raw: ${i} -> Final: ${s}`),"information_seeking"===s)a=await b(r,t);else{console.log("Handling personal reflection intent...");let{data:e}=await n.supabaseAdmin.from("chat_logs").select("role, content").eq("participant_id",r.id).order("created_at",{ascending:!1}).limit(9),i=(e||[]).reverse().map(e=>({role:"ai"===e.role?"assistant":"user",content:e.content}));i.push({role:"user",content:t});let s=r.profile_summary?`
[ユーザープロフィール要約]
${r.profile_summary}
`:"",l=`
${o}${s}
